...
from module.flax import RotaryEmbedding, Attention, MLP, RMSNorm
...
class LlamaTransformer(nnx.Module):
    def __init__(self, key, config: LlamaConfig, layer_idx: int | None = None):
        ...
        self.mlp = MLP(
            mlp_key, config.hidden_size, 
            config.intermediate_size, 
            config.act_fn, config.bias,
            dtype=config.dtype
        )
    ...
    def __call__(self, hidden_states: jax.Array, attention_mask: jax.Array, position_embedding: tuple[jax.Array]):
    ...
    def __init__(self, config: LlamaConfig, key):
        ...
        self.blocks = nnx.List([
            LlamaTransformer(key, config, layer_idx) \
                for layer_idx, key in enumerate(jax.random.split(transformer_key, config.num_hidden_layers))
        ])

    def __call__(self, input_ids: jax.Array, attention_mask: jax.Array | None = None, position_ids: jax.Array | None = None):
        ...
        if attention_mask is not None:
            ...
            if q_len > 1:
                causal_mask = nnx.combine_masks(
                    causal_mask,
                    nnx.make_causal_mask(input_ids)
                )
        else:
            causal_mask = None
        ...
        for layer in self.blocks[:self.num_hidden_layers]:
            hidden_states = layer(
                hidden_states, causal_mask, position_embedding
            )

        return self.layernorm(hidden_states)


class LlamaLanguageModel(KVCacheBase, LanguageModel):
    def __init__(self, config: LlamaConfig, key: jax.typing.ArrayLike=jax.random.key(42)):
